...
\begin{document}
    ...
    \section{導関数}
        ...
        \section{導関数の計算}
            ...
            \section{微分法諸定理}
                ...
                $\displaystyle(3)\lim_{x\to 0}\frac{\log x}{x}$\hspace{3mm}
                ...
                \begin{enumerate}\setcounter{enumi}{0}\renewcommand{\labelenumi}{(\arabic{enumi})}
                    ...
                    \item $\sin x$のマクローリン展開と$\cos x$のマクローリン展開と(2)から次の\textbf{オイラーの公式}\footnote{この公式は抜群に役に立つ。数学以外の様々な顔でも出し、人類の至宝と称される。覚えておいて損はないと断言できる。}を導け。
                        \begin{equation}
                            e^{ix}=\cos x+i\sin x
                        \end{equation}
                \end{enumerate}
            ...
            \paragraph{問7}$(1+x)^{\alpha}\quad(\alpha\text{は実数})$のマクローリン展開を求め、二項定理を証明せよ。
    ...
    \part{積分$\int$}
        ...
        \begin{screen}
            ...
            歴史的には定積分,微分,不定積分の順に発明されたというのだから、面白い。
        \end{screen}
    \clearpage
    \part{無限級数$\sum$}
    \clearpage
